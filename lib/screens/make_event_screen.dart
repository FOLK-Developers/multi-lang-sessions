...
class MakeEventScreen extends StatefulWidget {
  static const routeName = '/make-event';

  const MakeEventScreen({Key? key}) : super(key: key);

  @override
  _MakeEventScreenState createState() => _MakeEventScreenState();
}

class _MakeEventScreenState extends State<MakeEventScreen> {
      ...
      body: Form(
        ...
        child: SingleChildScrollView(
          child: Container(
            ...
            height: Utility().getSize(context).height - kToolbarHeight - 50,
            child: Column(
              children: [
                Expanded(
                  flex: 7,
                        ...
                        : ElevatedButton(
                            onPressed: () async {
                              if (_form.currentState!.validate()) {
                                startDate = fromISOString(startDateCtrl.text);
                                endDate = fromISOString(endDateCtrl.text);
                                int startEpoch = getEpoch(
                                    startDate,
                                    int.parse(
                                        startTimeCtrl.text.substring(0, 2)),
                                    int.parse(startTimeCtrl.text.substring(5)));
                                int endEpoch = getEpoch(
                                    endDate,
                                    int.parse(endTimeCtrl.text.substring(0, 2)),
                                    int.parse(endTimeCtrl.text.substring(5)));

                                updatedInfo.addAll({
                                  'start_time': startEpoch,
                                  'end_time': endEpoch,
                                  'creation_time': Utility.getCurrentEpoch,
                                });
                                print(
